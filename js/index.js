$(function(){
  ...
  var $btn=$(".min-nav a:first-child")
    ...
    //footer点击效果
    var $footerbtn =$('.directory-colum-title');
   	
    //console.log($footwinW)
    $footerbtn.click(function(){
 		var $footwinW=document.documentElement.clientWidth;
    	var $listH = $(this).siblings().children("li").length*24
    	var $liHeight = $(this).siblings().height();     //获取this同辈的ul高度，如果为0，点击高度变为$listH  	
    	//$(this).siblings().fadeToggle();
    	if($footwinW <768){
    		if(  $liHeight == 0  ){
	            $(this).siblings().animate({height:$listH})
	      	}else{
	            $(this).siblings().animate({height:0})       
	        }
    	}else {
    		//$(this).siblings().css("height",$listH)
    		$(".directory-colum-list").removeAttr("style")
    	}
